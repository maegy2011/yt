  ...
  try {
    ...
    for (const video of watchedVideos) {
      ...
      if (video.videoId.length > 11 || !/^[a-zA-Z0-9_-]+$/.test(video.videoId)) {
        console.log(`Found incorrect video ID: ${video.videoId}`)
        ...
        if (thumbnailMatch && thumbnailMatch[1]) {
          ...
          console.log(`Extracted correct video ID: ${correctVideoId}`)
          ...
          if (existingCorrect) {
            ...
            console.log(`Updated existing record for video ID: ${correctVideoId}`)
          } else {
            ...
            console.log(`Fixed video ID from ${video.videoId} to ${correctVideoId}`)
            fixedCount++
          }
          ...
          console.log(`Deleted record with invalid video ID: ${video.videoId}`)
          deletedCount++
        }
      }
    }
    ...
    console.log('Video ID fix completed:', message)
