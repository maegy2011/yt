...
export const confirmationMessages = {
  ...
  playlistLoaded: (title: string, count: number) => [
    `Loaded ${count} videos from "${title}"!`,
...
export function getConfirmationMessage(operation: keyof typeof confirmationMessages, args: any[] = []): string {
  ...
  if (typeof messages === 'function') {
    let result: any
    if (operation === 'search') {
      result = (messages as (count: number) => string[])(args[0] || 0)
    } else if (operation === 'playlistLoaded') {
      result = (messages as (title: string, count: number) => string[])(args[0] || '', args[1] || 0)
    } else {
      result = (messages as (...args: any[]) => string[])(...args)
    }
